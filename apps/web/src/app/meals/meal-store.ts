...
/**
 * Single source of truth for logged meal entries and every balance derivation
 ...
 * "net = intake − expenditure" rule is applied — so when exercise logging lands,
 * only the expenditure here changes and every view follows.
 *
...
export class MealStore {
  ...
  /** Local day of the user's earliest entry, or null — the data horizon's lower edge. */
  ...
  /** Daily expenditure. Just TDEE today; TDEE + exercise burn once that ships. */
  ...
  /** Fetch the earliest logged entry (data horizon). Cheap; call on page init. */
  ...
  /** Directional net balance for a single local day. */
  dayBalance(key: string): DayBalanceResult {
    return dayBalance(this.byDay().get(key) ?? [], this.expenditure());
  }
